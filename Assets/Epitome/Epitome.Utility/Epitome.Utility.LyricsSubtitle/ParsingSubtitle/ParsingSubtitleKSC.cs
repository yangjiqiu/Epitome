...
/// </summary>
public class ParsingSubtitleKSC : Singleton<ParsingSubtitleKSC>
{
	/// <summary>
	/// 歌曲		歌名
	/// </summary>
	...
	public static ParsingSubtitleKSC InitKsc(string LrcPath)
	{
		int row = 0;
		//KscWord对象
		//清除之前的歌曲歌词, 保持当前
		ParsingSubtitleKSC.Instance.Add.Clear();
		using (FileStream fs = new FileStream(LrcPath, FileMode.Open, FileAccess.Read, FileShare.Read))
		{
			...
			using (StreamReader sr = new StreamReader(fs, Encoding.Default))
			{
				...
				while ((line = sr.ReadLine()) != null)
				{
					...
					ParsingSubtitleKSC.KscWord kscWord = new ParsingSubtitleKSC.KscWord();
					#region ######################################合唱歌曲格式
					...
					{
						//获取tag标签的参数信息
						ParsingSubtitleKSC.Instance.listTags.Add(SplitTagInfo(line));
						#endregion

						#region ################################################歌词正文部分解析
					}
					else if (line.StartsWith(("karaoke.add")))
					{                                                                       //表示歌词正文部分
						if (SplitLyricStartTime(line) != null)
						{
							...
							ParsingSubtitleKSC.Instance.Add.Add(row, kscWord);
							row++;
						}
					}
					else
